...
export function yamlToJSON(yaml) {
	// The Tracker Format setting can be JSON. This hand-rolled parser only understands the YAML
	// subset emitted by jsonToYAML, so when the input is already JSON, parse it natively. (YAML
	// output from jsonToYAML never begins with "{"/"[", so this guard is unambiguous.)
	const trimmed = typeof yaml === "string" ? yaml.trim() : "";
	if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
		try {
			return JSON.stringify(JSON.parse(trimmed), null, 2);
		} catch (e) {
			// Not valid JSON after all; fall through to the YAML line parser below.
		}
	}
